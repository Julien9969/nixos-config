        ...
        embed() {
            case $1 in
                ...
                backup-server)
                    ...
                    color="15105570"
                    url="https://filebrowser.${secrets.main_domain}/files/NAS/backups/"
                    thumbnail=""
                    footer_text=""
                    footer_image=""
                    echo "$(json "$avatar_url" "$username" "$title" "$description" "$color" "$url" "$thumbnail" "$footer_text" "$footer_image")"
                ...
                backup-jellyfin)
                    ...
                    description="||test||\`\`\`scp -P 52222 -r trizotto@${secrets.ssh_host}:/media/NAS/backups/jellyfin.tgz <out-dir>\`\`\`\n> $(date +"%Y-%m-%d")\n> $(date +"%H:%M:%S")"
                    color="15105570"
                    url="https://filebrowser.${secrets.main_domain}/files/NAS/backups/"
                    thumbnail=""
                    footer_text=""
